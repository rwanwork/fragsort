...
#
#  Subversion date:  $Id: run-expt.pl 37 2010-12-16 09:04:59Z rwan $
#
use FindBin qw ($Bin);
use lib "$Bin";
...
foreach my $method (@methods) {
  for (my $k = 0; $k <= 100; $k += $INCR) {
    for (my $iter = 0; $iter < $REPEATS; $iter++) {
      ...
      $retval = callProgram ($REORDER, "--groupsize 2 --ordering order-frags.txt", $half_frags_fn, $out_half_frags_fn, $HALFFRAGS_TIME, $k);
      runGzipBzip ($out_half_frags_fn, $half_frags_size, $fastq_size, $HALFFRAGS_RESULTS, $HALFFRAGS_TIME, $method, $fn, $k, $iter);

      ##  Run re-order and then compress the identifier + qscores combination
      $retval = callProgram ($REORDER, "--groupsize 2 --ordering order-qscores.txt", $half_qscores_fn, $out_half_qscores_fn, $HALFQSCORES_TIME, $k);
      runGzipBzip ($out_half_qscores_fn, $half_qscores_size, $fastq_size, $HALFQSCORES_RESULTS, $HALFQSCORES_TIME, $method, $fn, $k, $iter);

      ##  Run re-order and then compress the one-id data
      $retval = callProgram ($REORDER, "--groupsize 4 --ordering order-frags.txt", $oneid_fn, $out_oneid_fn , $ONEID_TIME, $k);
      runGzipBzip ($out_oneid_fn, $oneid_size, $fastq_size, $ONEID_RESULTS, $ONEID_TIME, $method, $fn, $k, $iter);

      ##  Run re-order and then compress all of the data
      $retval = callProgram ($REORDER, "--groupsize 4 --ordering order-frags.txt", $fastq_fn, $out_fastq_fn , $FASTQ_TIME, $k);
      runGzipBzip ($out_fastq_fn, $fastq_size, $fastq_size, $FASTQ_RESULTS, $FASTQ_TIME, $method, $fn, $k, $iter);

      sleep (1);  ##  Sleep so that random number generator will get a seed from at least
                  ##  one second away.

      ##  Delete temporary ordering files
      unlink ("order-frags.txt", "order-qscores.txt");
...
run-expt.pl -- Run experiments

=head1 SYNOPSIS

B<run-expt.pl> --file sample --jobid 1 --quicksort
